...
The data is **still fully captured** (not discarded): in SDTM it goes in the visit structure (**SV**) with the unscheduled convention (`VISIT` = "UNSCHEDULED", flagged `VISITNUM`) — see [[SDTM, SDTM IG, and Conformance Rules]]. Analytically, unscheduled assessments are folded in via **visit windows** — mapped to the nearest planned analysis timepoint, or used as a confirmation / worst-case value — so they don't break the by-visit summaries built on the scheduled grid. The SoA often has an explicit **"Unscheduled" column** listing which assessments may be done as-needed.
